...
toComponents[x_.*y_,rul_]:=If[MatQ[x*y],reWrite[x*y/.rul],reWrite[(x*y*IdentityMatrix[Length[Select[Part[#,2]&/@rul,MatrixQ][[1]]]])/.rul]];
toComponents[x_\[Wedge]y_,rul_]:=If[MatQ[x\[Wedge]y],reWrite[x\[Wedge]y/.rul],reWrite[(x\[Wedge]y*IdentityMatrix[Length[Select[Part[#,2]&/@rul,MatrixQ][[1]]]])/.rul]];
...
DeclareMatrixForms[z__]:=Block[{x={{z}},xi,rxi,h,ht,k,min1,oldHeads,newHeads,tmp=AllMatrices},While[Head[x[[1,1]]]===List,x=First[x]];Unprotect[Transpose];Do[xi=x[[i]];If[Length[xi]<3,Print["Missing symbol for transposed matrix"];Abort[]];If[xi[[3]]===xi[[2]]||xi[[3]]===-xi[[2]],rxi={xi[[2]]},rxi=Rest[xi]];matForms[First[xi]]=Union[matForms[First[xi]],rxi];
AllMatrices=Union[AllMatrices,rxi];h=Head[xi[[2]]];If[First[xi]>0,AllMatForms=Union[AllMatForms,rxi],If[h=!=Symbol,ZeroHeads=Union[{h},ZeroHeads];If[Head[xi[[3]]]=!=Times,ZeroHeads=Union[{Head[xi[[3]]]},ZeroHeads]]]];If[h===Symbol,FormDegree[xi[[2]]]=First[xi];BasicMatQ[xi[[2]]]=True;BasicScalFormQ[xi[[2]]]=False;Transpose[xi[[2]]]=xi[[3]];If[xi[[3]]===-xi[[2]],tr[xi[[2]]]=0,If[xi[[3]]=!=xi[[2]],FormDegree[xi[[3]]]=First[xi];BasicMatQ[xi[[3]]]=True;BasicScalFormQ[xi[[3]]]=False;Transpose[xi[[3]]]=xi[[2]];tr[xi[[3]]]=tr[xi[[2]]]]],FormDegree[_h]=First[xi];BasicMatQ[_h]=True;BasicScalFormQ[_h]=False;ht=Head[xi[[3]]];If[ht===Times,min1=-1;ht=h;tr[h[k_]]=0,min1=1;If[ht=!=h,FormDegree[_ht]=First[xi];BasicMatQ[_ht]=True;BasicScalFormQ[_ht]=False;tr[ht[k_]]=tr[h[k]]]];Transpose[h[k_]]=min1 ht[k];Transpose[ht[k_]]=min1 h[k]],{i,Length[x]}];Protect[Transpose];
AllDifForms=Union[AllScalForms,AllMatForms];AllSymbols=Union[AllDifForms,AllMatrices];MatFormHeadList=Complement[Union[Head/@AllMatForms,MatFormHeadList],{Symbol}];oldHeads=AllMatHeads;AllMatHeads=Union[ZeroHeads,MatFormHeadList];newHeads=Complement[AllMatHeads,oldHeads];
HeadList=Union[Head/@AllSymbols];DifFormSymbols=Drop[AllDifForms,-Length[Union[ScalHeadList,MatFormHeadList]]];nodHeads=Union[nodHeads,ZeroHeads];k=Thread[Blank[Union[ScalHeadList,MatFormHeadList]]];FormVars=Flatten[{_Wedge,_d,Union[k,Bar[k]],y_|Bar[y_]/;MemberQ[DifFormSymbols,y],_tr}];ZeroMatSymbols=Drop[matForms[0],-Length[ZeroHeads]];ZeroVars=Flatten[{_Wedge,Union[Thread[Blank[ZeroHeads]],Bar[Thread[Blank[ZeroHeads]]]],y_|Bar[y_]/;MemberQ[ZeroMatSymbols,y],_tr}];If[tmp=!={},Print["OK"]]];


DeclareForms[z__]:=(DclFrmNoOK[z];Print["OK"])


DclFrmNoOK[z__]:=Block[{h,x={{z}},xi,rxi,k,oldHeads,newHeads},While[Head[x[[1,1]]]===List,x=First[x]];Do[xi=x[[i]];rxi=Rest[xi];Forms[First[xi]]=Union[Forms[First[xi]],rxi];
...
AllDifForms=Union[AllScalForms,AllMatForms];oldHeads=ScalHeadList;ScalHeadList=Complement[Union[Head/@AllScalForms,ScalHeadList],{Symbol}];newHeads=Complement[ScalHeadList,oldHeads];AllSymbols=Union[AllDifForms,AllMatrices];
HeadList=Union[Head/@AllSymbols];DifFormSymbols=Drop[AllDifForms,-Length[Union[ScalHeadList,MatFormHeadList]]];k=Thread[Blank[Union[ScalHeadList,MatFormHeadList]]];FormVars=Flatten[{_Wedge,_d,Union[k,Bar[k]],y_|Bar[y_]/;MemberQ[DifFormSymbols,y],_tr}]];


NoDif[z__]:=(nodHeads=Union[nodHeads,Flatten[{z}]];Print["OK"])
...
Default[Wedge]:=1;SetAttributes[Wedge,{Flat,OneIdentity}];Wedge[0,y__]=0;Wedge[x__,0]=0;Wedge[x_SeriesData,y_SeriesData]:=Block[{x$,y$,r1,r2,res,x3,y3},If[Last[x]===Last[y],x$=x;y$=y,x$=newSer$[x,LCM[Last[x],Last[y]]/Last[x]];y$=newSer$[y,LCM[Last[x],Last[y]]/Last[y]]];r1=x$[[-3]]+y$[[-3]];r2=Min[x$[[-2]]+y$[[-3]],x$[[-3]]+y$[[-2]]];If[Length[x$[[3]]]<x$[[-2]]-x$[[-3]],x3=Join[x$[[3]],Table[0,{x$[[-2]]-x$[[-3]]-Length[x$[[3]]]}]],x3=x$[[3]]];If[Length[y$[[3]]]<y$[[-2]]-y$[[-3]],y3=Join[y$[[3]],Table[0,{y$[[-2]]-y$[[-3]]-Length[y$[[3]]]}]],y3=y$[[3]]];Which[r2===r1,res={},r2-r1===x$[[-2]]-x$[[-3]],res=Sum[Map[Wedge[x3[[i]],#]&,Join[Table[0,{i-1}],Drop[y3,-i+1]]],{i,Length[x3]}],True,res=Sum[Map[Wedge[#,y3[[i]]]&,Join[Table[0,{i-1}],Drop[x3,-i+1]]],{i,Length[y3]}]];SeriesData[First[x$],x$[[2]],res,r1,r2,Last[x$]]]/;First[x]===First[y]&&x[[2]]===y[[2]];Wedge[y_,x_SeriesData]:=x/.x[[3]]->Map[Wedge[y,#]&,x[[3]]];Wedge[x_SeriesData,y_]:=x/.x[[3]]->Map[Wedge[#,y]&,x[[3]]];Wedge[x__,y_Plus]:=Plus@@Map[Wedge[x,#]&,List@@y];Wedge[x_Plus,y__]:=Plus@@Map[Wedge[#,y]&,List@@x];Wedge[z__,Times[x_,y_]]:=Times[x,Wedge[z,y]]/;NumericQ[x]||(FormDegree[x]===0&&!MatQ[x]);Wedge[Times[x_,y_],z__]:=Times[x,Wedge[y,z]]/;NumericQ[x]||(FormDegree[x]===0&&!MatQ[x]);Wedge[x_^n_.,y_]:=x^n*y/;FormDegree[x]===0&&!MatQ[x];Wedge[y_,x_^n_.]:=x^n*y/;FormDegree[x]===0&&!MatQ[x];Wedge[x_,y___,x_]:=0/;OddQ[FormDegree[x]]&&ScalFormQ[x];Wedge[x__,y__]:=(-1)^(FormDegree[x]*FormDegree[y])*y\[Wedge]x/;MatQ[x]&&ScalFormQ[y];Wedge[x__]:=Block[{doubL=Transpose[{FormDegree/@{x},{x}}]},Signature[Select[doubL,OddQ[First[#]]&]]Wedge@@Map[Last[#1]&,Sort[doubL]]]/;Union[BasicScalFormQ/@{x}]==={True}&&Map[Last[#1]&,Sort[Transpose[{FormDegree/@{x},{x}}]]]=!={x};Wedge[x_List,y_List]:=If[(FormDegree[x]===0&&!MatQ[Last[Union[Flatten[x]]]])||(FormDegree[y]===0&&!MatQ[Last[Union[Flatten[y]]]]),Dot[x,y],Inner[Wedge,x,y]];Wedge[x_,y_List]:=If[FormDegree[x]===0||(FormDegree[y]===0&&!MatQ[Last[Union[Flatten[y]]]]),x *y,Map[Wedge[x,#]&,y]]/;!BasicMatQ[x];Wedge[x_List,y_]:=If[FormDegree[y]===0||(FormDegree[x]===0&&!MatQ[Last[Union[Flatten[x]]]]),x*y,Map[Wedge[#,y]&,x]]/;!BasicMatQ[y];


simpRules = {x_.*Cos[h_]^2+x_.*Sin[h_]^2->x};varList={};


SetAttributes[reWrite,{Listable}];reWrite[0]=0;reWrite[x_Equal]:=Equal[reWrite[First[x]-Last[x]],0];reWrite[x_Rule]:=Rule[First[x],reWrite[Last[x]]];reWrite[x_RuleDelayed]:=Rule[First[x],reWrite[x[[2]]]]/.Rule->RuleDelayed;reWrite[x_SeriesData]:=(x/.x[[3]]->reWrite[x[[3]]]);reWrite[x_Condition]:=Condition[reWrite[x[[1]]],x[[2]]];reWrite[x_]:=(Collect[x/.simpRules,FormVars,bestFacRul]/.simpRules)/;FormDegree[x]>0;reWrite[x_]:=(Collect[x/.simpRules,ZeroVars,bestFacRul]/.simpRules)/;MatQ[x];reWrite[x_]:=bestFacRul[x/.simpRules]/.simpRules;
...
FlatOuter[x_?MatrixQ,y_?MatrixQ]:=
  Partition[Flatten[Transpose[Outer[Times,x,y],{1,3,2,4}]],Dimensions[x][[2]]*Dimensions[y][[2]]];
...
CHrul[n_,x_:$]:=Block[{cn=CHeq[x,n],cn0,rul,tmp,r1,r2},If[FormDegree[x]>0,Print[x," is not a 0-form matrix"];Return[]]; matDimension=n;rul=First[Solve[cn==cn0,Wedge@@Table[x,{n}]]]/.cn0->tr[cn]/n;rul=reWrite[rul];If[x===$,tmp=MapAt[HoldPattern,rul/.Rule->RuleDelayed,{1,1}];r1=tmp[[1,1]];r2=r1/.$->Pattern[$,Blank[]];tmp={Rule[r2,tmp[[1,2]]/;MatQ[$]&&FormDegree[$]===0&&matDimension===n]}/.Rule->RuleDelayed,tmp=rul]];


detTOtr[n_,A_:$]:=Block[{expr=CHrul[n][[1,2,1]]},(-1)^(1+matDimension) Plus@@Select[List@@expr,MatQ[#1]===False&]/.$->A];


Protect[CHeq,CHrul,CHrule,detTOtr];


TrigRules={x_.*Cos[h_]^2+x_.*Sin[h_]^2->x,x_.*Cot[h_]^2+y_.*Csc[h_]^2:>y/;x+y===0,(Cot[h_]-Csc[h_]) (Cot[h_]+Csc[h_])->-1};
HypTrigRules={x_.*Cosh[h_]^2+y_.*Sinh[h_]^2:>x/;x+y===0,x_.*Coth[h_]^2+y_.*Csch[h_]^2:>x/;x+y===0,(Cosh[x_]-Sinh[x_]) (Cosh[x_]+Sinh[x_])->1,(Coth[h_]-Csch[h_]) (Coth[h_]+Csch[h_])->1};
imgTrigRules={(Cos[x_]-I Sin[x_]) (Cos[x_]+I Sin[x_])->1,( Cos[x_]+I Sin[x_]) (I Cos[x_]+Sin[x_])->I,(Cos[x_]-I Sin[x_])(-I Cos[x_]+Sin[x_])->-I};
RootRules={(-1+x_)(1+x_)/Sqrt[1-x_^2]->-Sqrt[1-x^2],(-1+x_)(1+x_)/Sqrt[x_^2-1]->Sqrt[x^2-1],(-x_.+Sqrt[y_])(x_.+Sqrt[y_]):>y-x^2,(x_.-Sqrt[y_])(x_.+Sqrt[y_]):>-y+x^2};
BasicRules=Join[TrigRules,HypTrigRules,RootRules,imgTrigRules];$RGTCversion=375;
...
testSER[x_,y_String,z_:""]:=Block[{tmp,k},If[FreeQ[x,SeriesData]||(Dim===2&&TensRnk[x]<4),y<>z,tmp=Sort[Union[Select[Flatten[Which[TensRnk[x]===4,Raise[x,1,2],TensRnk[x]>1,Raise[x,1],True,x]],Head[#]===SeriesData&]],#1[[-2]]/#1[[-1]]<#2[[-2]]/#2[[-1]]&];k=" to order "<>ToString[(tmp[[1,-2]]-1)/tmp[[1,-1]]];y<>If[Length[tmp]>1,k=k<>" or higher",k]]];
...
dotAll[x_,y_,indPr_]:=Block[{s$,allInd,sumInd,tmp$=FormDegree[0]},allInd=s$/@Range[Length[indPr]];allInd={allInd,allInd/.Map[Rule[#[[1]],#[[2]]]&,RotateLeft/@indPr]};sumInd=Transpose[{First[allInd],Dimensions[x]}];If[tmp$===0&&FormDegree[x]>0&&FormDegree[y]>0,tmp$=Wedge,tmp$=Times];Sum@@{Unevaluated[tmp$[x[[Sequence@@First[allInd]]],y[[Sequence@@allInd[[2]]]]]],Sequence@@sumInd}];


Contract[x_,indPr__]:=Block[{rnk1=TensRnk[x],indL=Flatten[{indPr}]},If[!And@@Map[SameQ[Dimensions[x][[#[[1]]]],Dimensions[x][[#[[2]]]]]&,{indPr}],er$Wrn[3];Return[]];If[Length[Union[indL]]<Length[indL]||Length[indL]>rnk1||Sort[indL][[-1]]>rnk1,er$Wrn[2];Return[],OKindCon[x,Sort[Sort/@{indPr}]]]];
...
LieDerv[h_List,x_,UpList_:{}]:=Module[{k=TensRnk[x],hjdU,hUjdi$,ASgam=GUdd-Transpose[GUdd,{1,3,2}]},
If[Length[Dimensions[h]]=!=1,stPrint["First argument not a vector!"];Return[]];
If[Length[UpList]>0&&(Length[Flatten[UpList]]>k||Sort[Flatten[UpList]][[-1]]>k),er$Wrn[2];Return[]];
hjdU=Array[p$D[h,#]&,{Dim}];hUjd=Transpose[hjdU]+ASgam.h;hjdU=Transpose[hUjd];
FaSi[h.Array[p$D[x,#]&,{Dim}]+If[Head[x]===List,Sum[Which[i$===1,If[MemberQ[UpList,i$],-hUjd.x,hjdU.x],
...
Protect[cDcomp,CF3d,ConOne,c$D,dot$2,LieDerv,dotAll,dotOne,epsilon,FacSimp,FaSi,GenCoef,indexList,indx$Ruls,inn,metric,M$F,MoveIndex,newPrList,numb1,numb2,OKindCon,redPlus,redTimes,RiemSym,zeroTensor,testSER,testSolve,tr$2,OrdN];
